...
function upVoteComment(url,request){
  ...
  const commentUsername = request.body && request.body.username;
  let currentComment = (database.comments[commentId]);

  if (currentComment && database.users[commentUsername] ){

    currentComment = upvote(currentComment,commentUsername);
    response.body = {comment: currentComment};
    response.status = 200;
...
function downVoteComment(url,request){
  ...
  if (currentComment && database.users[commentUsername] ){
    ...
    currentComment = downvote(currentComment,commentUsername);
    response.body = {comment: currentComment};
    response.status = 200;
...
function updateComment(url,request){
  const reponse = {};
  const currentCommentId = Number(url.split('/').filter(segment => segment)[1]);
  const oldComment = database.comments[currentCommentId];
  const newComment = request.body && request.body.comment;
  //console.log(request.body);
  if(!oldComment){
    reponse.status = 404;
  } else if (!newComment || !newComment.body){
    reponse.status = 400
  } else {
    oldComment.body = newComment.body;
    reponse.body = {article: oldComment};
    reponse.status = 200;
  }
  return reponse;
}
...
function deleteComment(url){
  const currentCommentId = Number(url.split('/').filter(segment => segment)[1]);
  const currentComment = database.comments[currentCommentId];
  const reponse = {};

  if (!currentCommentId || !currentComment){
    reponse.status = 404;
  } else {
    const deleteCommentId = database.articles[currentComment.articleId];
    const articleCommentId = deleteCommentId.commentIds;

    const commentUsername = currentComment.username;
    ...
    database.comments[currentCommentId] = null;
...
function createComment(url, request){
  ...
  const reponse = {};
  //console.log(Object.keys(commentInfo));
  if(commentInfo && (commentInfo.hasOwnProperty('comment')) && (commentInfo.comment.body) &&
       database.articles[commentInfo.comment.articleId] && database.users[commentInfo.comment.username]){
    const newComment = {
